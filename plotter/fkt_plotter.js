...
window.addEventListener("DOMContentLoaded", function() {
    ...
    var fkt_plotter = function() {
            ...
            for (var i = 0; i < daten.length; i++)
                if (!isNaN(daten[i][0].y)) plt.scale(daten[i]);
            plt.frame(50, 35, "x", "y");
            for (var i = 0; i < daten.length; i++)
                if (!isNaN(daten[i][0].y)) plt.plot(daten[i], cols[i]);
    ...
    var i, j, e_plotarea, e_wertetabelle, e_fkt = [],
        e_xmin, e_xmax, plt, e_jsfkt, e_vdeffkt;
    var npt = 1000;
    var current = 0;
